...
namespace RE
{
	...
	uint32_t Actor::GetSpellCount(Actor* thisActor)
	{
		if (!thisActor)
			return NULL;
	...
	SpellItem* Actor::GetNthSpell(Actor* thisActor, uint32_t a_num)
	{
		if (!thisActor)
			return NULL;

		return thisActor->addedSpells.get(a_num);
	}

	TESForm* Actor::GetEquippedShout(Actor* thisActor)
	{
		if (!thisActor)
			return NULL;
